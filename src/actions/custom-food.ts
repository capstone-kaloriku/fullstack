...
function mapRailwayDataToResult(
  ...
  return {
    ...
    base_portion_gram: safeNumber(
      data.base_portion_gram ?? data.portion_gram,
      100,
    ),
...
export async function validateFoodWithAI(
  ...
  try {
    ...
    if (detail.found && detail.data) {
      return {
        success: true,
        data: mapRailwayDataToResult(detail.data, foodName),
      };
    }
    ...
    if (search.total > 0 && search.results[0]) {
      return {
        success: true,
        data: mapRailwayDataToResult(search.results[0], foodName),
      };
    }
    ...
    return {
      success: false,
      error:
        "Gagal memvalidasi makanan. Pastikan koneksi ke server AI tersedia.",
    };
  }
}
